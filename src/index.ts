...
function getAvailableModels(): ModelInfo[] {
  ...
  try {
    const rawOutput = execSync(`"${agyExec}" models`, {
      env: process.env,
      timeout: 10000,
      encoding: "utf-8",
    }).toString();
    ...
    if (msg.includes("ENOENT")) {
      throw new Error(
        `Google Antigravity CLI ('agy') is not installed or not found on PATH. ` +
        `Please install Google Antigravity CLI, ensure 'agy' is on your PATH, and run 'agy auth login'.`
      );
    }
    throw new Error(
      `Failed to fetch Google Antigravity models: ${msg}. Please ensure 'agy' is authenticated via 'agy auth login'.`
    );
  }

  if (availableModels.length === 0) {
    throw new Error(
      "No models returned by Google Antigravity CLI ('agy models'). Please run 'agy auth login' to authenticate."
    );
  }
